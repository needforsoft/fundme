		...
		<form method="POST" action="{{ url('donate', $response->id) }}" enctype="multipart/form-data" id="formDonation">
			...
			<div class="form-group">
				...
				<div class="input-group has-success">
					...
					<input type="number" min="{{$settings->min_donation_amount}}" autocomplete="off" id="onlyNumber"
						class="form-control input-lg" name="amount" @if( isset($pledge) )readonly='readonly' @endif
						value="@if( isset($pledge) ){{$pledge->amount}}@endif"
						placeholder="{{trans('misc.minimum_amount')}} @if($settings->currency_position == 'left') {{$settings->currency_symbol.$settings->min_donation_amount}} @else {{$settings->min_donation_amount.$settings->currency_symbol}} @endif {{$settings->currency_code}}">
				</div>
			</div>
			...
			<div class="form-group">
				...
				<input type="text" id="cardholder-name" value="@if( Auth::check() ){{Auth::user()->name}}@endif"
					name="full_name" class="form-control input-lg"
					placeholder="{{ trans('misc.first_name_and_last_name') }}">
			</div><!-- /. End-->

			<!-- Start -->
			<div class="form-group">
				<label>{{ trans('auth.email') }}</label>
				<input type="text" id="cardholder-email" value="@if( Auth::check() ){{Auth::user()->email}}@endif"
					name="email" class="form-control input-lg" placeholder="{{ trans('auth.email') }}">
			</div><!-- /. End-->
			...
			<div class="row form-group">
				...
				<div class="col-xs-6">
					...
					<select id="country" name="country" class="form-control input-lg">
						...
						<option @if( Auth::check() && Auth::user()->countries_id == $country->id ) selected="selected"
				...
				<div class="col-xs-6">
					...
					<input type="text" id="postal_code" value="{{ old('postal_code') }}" name="postal_code"
						class="form-control input-lg" placeholder="{{ trans('misc.postal_code') }}">
				</div><!-- /. End-->
			...
			<div class="form-group">
				<input type="text" id="comment" value="{{ old('comment') }}" name="comment"
					class="form-control input-lg" placeholder="{{ trans('misc.leave_comment') }}">
			</div><!-- /. End-->
			...
			<div class="btn-block display-none" id="bankTransferBox">
				...
				<div class="row form-group">
					...
					<div class="col-sm-12">
						...
						<textarea name="bank_transfer" rows="4" class="form-control input-lg"
							placeholder="{{ trans('admin.bank_transfer_info') }}"></textarea>
					</div><!-- /. End-->
				</div><!-- row form-control -->
			...
			<div class="box-footer text-center">
				...
				<button type="submit" id="buttonDonation"
					class="btn-padding-custom btn btn-lg btn-main custom-rounded">{{ trans('misc.donate') }}</button>
		...
		<div class="panel panel-default">
			<div class="panel-body">
				<h3 class="btn-block margin-zero" style="line-height: inherit;">
					{{trans('misc.seleted_pledge')}} <small><a
							href="{{url('donate',$response->id)}}">{{trans('misc.remove')}}</a></small>
				</h3>
				<h3 class="btn-block margin-zero" style="line-height: inherit;">
		...
		<div class="panel panel-default">
			<div class="panel-body">
				<div class="media none-overflow">
					...
					<span
						class="btn-block text-center margin-bottom-10 text-muted"><strong>{{trans('misc.organizer')}}</strong></span>
		...
		<div class="panel panel-default">
			<div class="panel-body">
				<img class="img-responsive img-rounded" style="display: inline-block;"
					src="{{url('public/campaigns/small',$response->small_image)}}" />
			</div>
		</div>
				...
				<form action="{{url('charge')}}" method="post" id="payment-form">
					...
					<div class="form-row text-center">
						...
						<button id="card-button"
							class="btn btn-lg btn-main custom-rounded">{{trans('misc.donate')}}</button>
					</div>
...
function handleServerResponse(response) {
  if (response.error) {
    $('.popout').addClass('popout-error')
	.html('{{trans('misc.error')}}')
	.fadeIn('500')
	.delay('5000')
	.fadeOut('500');
