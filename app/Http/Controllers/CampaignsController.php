	...
	public function withdrawal()
	{
		...
		if (!empty($res) && empty($WithdrawalsExists)) {
			...
			foreach ($res->donations as $key) {
				...
				if ($key->payment_gateway == 'PayPal' || $key->payment_gateway == 'Paypal') {
				...
				} elseif ($key->payment_gateway == 'Stripe') {
					$fee      = config('commissions.stripe_fee');
					$cents   =  config('commissions.stripe_cents');

					$_amountGlobal = $key->donation - ($key->donation * $fee / 100) - $cents;
					$funds += $_amountGlobal - ($_amountGlobal * $this->settings->fee_donation / 100);
				} elseif ($key->payment_gateway == 'Bank Transfer') {
					$funds += $key->donation - ($key->donation * $this->settings->fee_donation / 100);
				}
			} //<-- foreach
